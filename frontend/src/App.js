...
import { HeaderBannerAd, InContentAd, SidebarRectangleAd, MobileBannerAd } from './components/AdSenseAd';

const BACKEND_URL = process.env.REACT_APP_BACKEND_URL;
const API = `${BACKEND_URL}/api`;

const PlatformFilter = ({ selectedPlatform, onPlatformChange, currentUser, onShowPricing }) => {
  const platforms = [
    ...
    <div className="flex flex-wrap justify-center gap-3 mb-8">
      ...
      {(!currentUser || currentUser.subscription_tier === 'free') && (
        <button
          ...
          className="px-4 py-2 rounded-full border-2 border-dashed border-yellow-400 text-yellow-600 hover:bg-yellow-50 transition-colors font-medium"
        >
...
const SubscriptionModal = ({ isOpen, onClose, onSubscribe }) => {
  ...
  const [methods, setMethods] = useState(['email']); // Default to email
  ...
  const handleSubmit = async (e) => {
    ...
    if (!methods.includes('email')) {
      setMessage('Email delivery is required');
      return;
    }
    ...
    try {
      // Subscribe to email notifications via new API
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/emails/subscribe`, {
        ...
        body: JSON.stringify({
          email: email.trim(),
          name: name.trim() || 'Viral Video Fan',
      ...
      if (response.ok) {
        ...
        // Call the original onSubscribe for backward compatibility
        onSubscribe({
          email: email.trim(),
            ...
            <div className="bg-gray-50 p-3 rounded-lg">
              <label className="flex items-center cursor-pointer">
                ...
                <span className="flex-1">
                  📧 <strong>Daily Email</strong> - Get viral videos delivered to your inbox every morning
                </span>
              </label>
            </div>
            <p className="text-xs text-gray-500 mt-2">
              More delivery options (Telegram, WhatsApp) coming soon!
            </p>
          </div>
      ...
      {!showPricingPage && (
        <>
          ...
          <main className="container mx-auto px-4 py-8">
            {/* Usage Tracking Banner for Free Users */}
            {(!currentUser || currentUser.subscription_tier === 'free') && (
              <div className="mb-8 bg-gradient-to-r from-purple-600 to-pink-600 text-white p-4 rounded-lg shadow-lg">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    <Crown className="h-5 w-5 mr-2" />
                    <span className="font-semibold">
                      Free Tier: {videos.length}/40 videos today
                    </span>
                  </div>
                  <button
                    onClick={() => setShowPricingPage(true)}
                ...
                {videos.length >= 35 && (
                  <div className="mt-2 text-sm bg-red-500 bg-opacity-20 p-2 rounded">
                    ⚠️ Almost at your daily limit! Upgrade to Pro for 100+ videos daily
                  </div>
                )}
              </div>
            )}
            ...
            {(!currentUser || currentUser.subscription_tier === 'free') && (
              <div className="mb-8 bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-semibold text-yellow-800">🔒 Premium Features Available</h3>
                    <p className="text-yellow-700 text-sm">Advanced analytics, API access, unlimited videos & more</p>
                  </div>
                  <button
                    onClick={() => setShowPricingPage(true)}
            ...
            {/* Header Banner Ad - Top Revenue Position */}
            {(!currentUser || currentUser.subscription_tier === 'free') && (
              <div className="mb-8">
                <HeaderBannerAd adTest={true} />
              </div>
            )}
              ...
              <div className="mb-8 bg-blue-50 border border-blue-200 p-6 rounded-lg">
                <div className="flex items-start justify-between">
                  <div className="flex items-start">
                    ...
                    <div>
                      <h3 className="font-semibold text-blue-800 mb-2">🔒 API Access (Pro Feature)</h3>
                      <p className="text-blue-700 text-sm mb-2">
                        Integrate Viral Daily into your apps with our powerful API
                      </p>
                      <ul className="text-blue-600 text-sm space-y-1">
                        <li>• 1,000+ API calls per month</li>
                        <li>• Real-time viral video data</li>
                        <li>• JSON/REST API endpoints</li>
            ...
            {!loading && !error && (
              <>
                ...
                {/* In-Content Ad - High Engagement Position */}
                {(!currentUser || currentUser.subscription_tier === 'free') && videos.length > 0 && (
                  <div className="mb-8">
                    <InContentAd adTest={true} />
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {videos.map((video, index) => (
                    <Fragment key={video.id || index}>
                      ...
                      {/* Inject ads between videos for free users - Every 4th video */}
                      {(!currentUser || currentUser.subscription_tier === 'free') && 
                       (index + 1) % 4 === 0 && 
          ...
          <footer className="bg-gradient-to-r from-gray-800 to-gray-900 text-white py-12 mt-16">
            <div className="container mx-auto px-4 text-center">
              ...
              <p className="text-sm text-gray-400 mb-6">
                Aggregating the best viral videos from YouTube, TikTok, and Twitter
              </p>
